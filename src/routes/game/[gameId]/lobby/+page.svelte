<script lang="ts">
	...
	const { enhance } = superForm(data.kickUserForm, {
...
<div class="card variant-ghost">
	...
	<section class="p-4 w-64 grid grid-cols-[auto_1fr_auto] space-x-2 relative">
		{#each $gameState.players as playerId (playerId)}
			...
			{:else}
				<div
					class="place-self-center rounded-full aspect-square w-2 border relative right-1"
